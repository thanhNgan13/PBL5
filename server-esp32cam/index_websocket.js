...
const ESP32WebSocket = new WebSocket.Server({ noServer: true });
...
function broadcastImage(id, imgBase64, image) {
  ESP32WebSocket.clients.forEach(function (client) {
...
server.on("upgrade", (request, socket, head) => {
  ...
  if (pathname === "/control") {
    ESP32WebSocket.handleUpgrade(request, socket, head, (ws) => {
      ESP32WebSocket.emit("connection", ws, request, "/control");
    });
  } else if (pathname === "/display") {
    ESP32WebSocket.handleUpgrade(request, socket, head, (ws) => {
      ESP32WebSocket.emit("connection", ws, request, "/display");
    });
  } else {
    socket.destroy();
  }
...
ESP32WebSocket.on("connection", (ws, request, path) => {
  if (path === "/display") {
